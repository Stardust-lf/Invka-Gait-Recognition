...
class MyModel(tf.keras.Model):
    def __init__(self,initializer):
        ...
        self.mask = pre_mask
        self.kopMatrix = tf.Variable(
            initializer(shape=[FIGURE_SEZE * FIGURE_SEZE, FIGURE_SEZE * FIGURE_SEZE], dtype=tf.float32),
            name='Koopman Matrics', trainable=True)
        ...
        #self.dense2 = tf.keras.layers.Dense(units=NUM_DENSE_UNITS, name='dense2',trainable=not self.fine_tune)
        # self.dense5 = tf.keras.layers.Dense(units=NUM_DENSE_UNITS, name='dense5',trainable=not self.fine_tune)
        self.dense3 = tf.keras.layers.Dense(units=2048, name='dense3', trainable=not self.fine_tune)
        #self.dense4 = tf.keras.layers.Dense(units=NUM_DENSE_UNITS, name='dense4',trainable=not self.fine_tune)
        # self.dense6 = tf.keras.layers.Dense(units=NUM_DENSE_UNITS, name='dense6',trainable=not self.fine_tune)
        # self.BatchNormalization1 = tf.keras.layers.BatchNormalization(name='Norm1',trainable=not self.fine_tune)
        self.BatchNormalization2 = tf.keras.layers.BatchNormalization(name='Norm2', trainable=not self.fine_tune)
        # self.BatchNormalization3 = tf.keras.layers.BatchNormalization(name='Norm3',trainable=not self.fine_tune)
        self.BatchNormalization4 = tf.keras.layers.BatchNormalization(name='Norm4', trainable=not self.fine_tune)
        #self.BatchNormalization5 = tf.keras.layers.BatchNormalization(name='Norm5',trainable=not self.fine_tune)
        #self.BatchNormalization6 = tf.keras.layers.BatchNormalization(name='Norm6',trainable=not self.fine_tune)
    ...
    def call(self, inputs,indexs):
        ...
        proce_phiX = tf.reshape(phiX,shape=[-1,STD_LEN,FIGURE_SEZE*FIGURE_SEZE])
        #print(proce_phiX.shape)
...
def comp_mat(tag,angle):
    ...
    # with open('ckpt_encoder_4096_denseKop', 'wb+') as f:
    #     train_var = model.trainable_variables
    #     pickle.dump([train_var[0],train_var[1]],f)
    #     print('aaa')
    model.summary()
    print(len(data))
    for j in range(124):
...
if __name__ == '__main__':
    data = load_variavle('data124')[0][0]
    ...
    optimizer = optimizers.Adagrad(learning_rate=0.001, epsilon=1e-4)
    model = MyModel(tf.random_normal_initializer(mean=1., stddev=2.))
    model.fine_tune = False
    model.batch_size = 1
    # model([data[0]],[0])
    # print('loading checkpoint')
    # model.load_weights('coder_fin_nm_withoutLoss2')
    # with open('ckpt_encoder_4096_denseKop', 'wb+') as f:
    #     train_var = model.trainable_variables
    #     pickle.dump([train_var[0],train_var[1]],f)
    #     print('aaa')
    ...
    while(epoch<=2000):
        epoch += 1
        loss0, loss1, loss2 = train_step([0], [data], model, optimizer)
        if epoch%100== 0:
            print(loss1+loss2)
            #print('Training epoch{3} for sample {4} cycle {5}(of{6}) loss0:{0}  loss1:{1}  loss2:{2}'.format(loss0, loss1, loss2, epoch, j, i, len(sample)))
    print('Sample {} has been trained')
    weigts_parm = model.trainable_variables
    kop_matrix = weigts_parm[0].value()
    kop_matrix = kop_matrix.numpy()
    print(kop_matrix.shape)
    model.save_weights('coder_fin_4096')
    # with open('matrix64nm_sigLoss/sample{0}_cycle{1}'.format(j,i),'wb+') as f:
    #     pickle.dump(kop_matrix,f)
